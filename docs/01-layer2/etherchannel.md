...
여러 물리적 이더넷 링크를 **하나의 논리 인터페이스(Port-Channel)** 로 묶어 대역폭 집계와 링크 이중화를 동시에 제공하는 기술. IEEE 802.3ad(LACP) 표준과 Cisco 독점 PAgP를 지원한다.

## 특징

- **(대역폭 집계)** 멤버 링크 수에 비례하여 논리 인터페이스의 총 대역폭을 선형으로 확장
- **(STP 루프 차단 우회)** 논리 인터페이스 단위로 STP가 동작하여 모든 멤버 링크를 동시에 활용 가능
- **(투명한 장애 복구)** 멤버 링크 장애 시 STP 재수렴 없이 나머지 링크로 즉시 트래픽 이동
